...
function launchEmulator(apiLevel, target, arch, profile, cores, ramSize, heapSize, sdcardPathOrSize, diskSize, avdName, forceAvdCreation, emulatorBootTimeout, port, emulatorOptions, disableAnimations, disableSpellChecker, disableLinuxHardwareAcceleration, enableHardwareKeyboard) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield waitForDevice(port, emulatorBootTimeout, locale);
 ...
 */
function waitForDevice(port, emulatorBootTimeout, locale) {
    return __awaiter(this, void 0, void 0, function* () {
        let booted = false;
        ...
        if (locale) {
            ...
            while (!localeChanged) {
                try {
                    let result = '';
                    yield exec.exec(`/bin/bash -c "adb -s emulator-${port} logcat -d | grep 'Pair{${locale}' | wc -l | tr -d ' '"`, [], {
                        listeners: {
                            stdout: (data) => {
                                result += data.toString();
                            },
